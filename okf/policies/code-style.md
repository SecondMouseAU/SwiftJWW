...
tags: [policy, style, swift, agents]
...
style-lint gate. `orphaned_doc_comment` is the one rule left that catches something
...
Why: this repo is the ecosystem's second pilot for the wider code-style policy (after
`OCCTSwiftScripts`), chosen for the same reason: small enough (~699 Swift lines across ~6 files)
to sweep into full compliance in one PR rather than needing a gradual adoption mechanism. No
first-party C++ exists here, so `clang-format` is out of scope for this repo entirely. Full
...
Filed and tracked as [SwiftJWW#9](https://github.com/SecondMouseAU/SwiftJWW/issues/9).
