...
struct SwiftJWWTests {
    ...
    struct Builder {
        ...
        mutating func header() {
            ...
            emptyStr()  // memo
            u32(0)
            u32(0)  // zumen, writeGLay
            for _ in 0..<16 {
                zeros(4 + 4 + 8 + 4)
                for _ in 0..<16 { zeros(8) }
            }  // layer-groups
            zeros(14 * 4)
            zeros(5 * 4)
            zeros(4)
            zeros(4)  // dummy, sunpou, dummy1, maxDrawWid
            zeros(8 * 2)
            zeros(8)
            zeros(4)
            zeros(4)
            ...
            zeros(8 * 2)
            zeros(8 * 2)
            for _ in 0..<(16 * 16) { emptyStr() }
            for _ in 0..<16 { emptyStr() }  // names
            zeros(8)
            zeros(8)
            zeros(4)
            zeros(8)  // kage...
            zeros(8)
            zeros(8)  // tenkuu (v>=300)
            zeros(4)
            zeros(8)
            zeros(8 * 2)
            zeros(8)
            ...
            zeros(8 * 10)
            zeros(8)  // fukusen, ryo
            for _ in 0..<10 { zeros(8) }
            for _ in 0..<10 { zeros(16) }  // Pen, PrtPen
            for _ in 0..<8 { zeros(16) }
            for _ in 0..<5 { zeros(20) }
            for _ in 0..<4 { zeros(16) }  // LType
            zeros(4 * 11)
            zeros(8)
            zeros(4 * 3)
            zeros(8 * 5)
        ...
        mutating func base(penStyle: Int = 1, color: Int = 2, layer: Int = 0) {
            ...
            u16(layer)
            u16(0)
            u16(0)  // v>=351 → penWidth present
        }
        mutating func str(_ s: String) {
            let a = Array(s.utf8)
            u8(a.count)
            d.append(contentsOf: a)
        }
        mutating func classTag(_ name: String) {
            u16(0xFFFF)
            u16(0x2bc)
            u16(name.utf8.count)
        ...
        mutating func senFields(
            _ x1: Double, _ y1: Double, _ x2: Double, _ y2: Double, layer: Int = 0
        ) {
            base(layer: layer)
            f64(x1)
            f64(y1)
        ...
        mutating func mojiFields(_ text: String, layer: Int = 0) {
            base(layer: layer)
            f64(0)
            f64(0)
            f64(0)
        ...
        mutating func tenFields(_ x: Double, _ y: Double, layer: Int = 0) {
            base(layer: layer)
            f64(x)
            f64(y)
            u32(0)
    ...
    static func sample() -> Data {
        ...
        b.u16(0x2bc)
        b.u16(8)
        b.d.append(contentsOf: Array("CDataSen".utf8))
        b.base(layer: 5)
        b.f64(0)
        b.f64(0)
        b.f64(10)
        ...
        b.u16(0x2bc)
        b.u16(9)
        b.d.append(contentsOf: Array("CDataEnko".utf8))
        b.base(layer: 7)
        b.f64(3)
        b.f64(4)
        b.f64(2)
    ...
    func reads() throws {
        ...
        guard case .line(let a, let bb, let layer, _) = dwg.entities[0] else {
            Issue.record("not a line")
            return
        }
        #expect(a.x == 0 && bb.x == 10 && layer == 5)
        guard case .arc(let c, let r, _, let sweep, _, let ratio, _, _, _) = dwg.entities[2] else {
    ...
    func dxf() throws {
        let dwg = JWW.Drawing(
            ...
            entities: [
                ...
                .arc(
                    center: .init(x: 0, y: 0), radius: 5, start: 0, sweep: 2 * .pi, tilt: 0,
                    ratio: 1, full: true, layer: 0, color: 1),
                .arc(
                    center: .init(x: 0, y: 0), radius: 5, start: 0, sweep: .pi, tilt: 0, ratio: 1,
                    full: false, layer: 0, color: 1),
                .point(at: .init(x: 2, y: 3), layer: 0, color: 1),
                .text(
                    at: .init(x: 0, y: 0), height: 2.5, width: 2.5, angleRad: 0, string: "AB",
                    layer: 0, color: 7),
            ], counts: .init())
    ...
    func arcTilt() throws {
        ...
        let dwg = JWW.Drawing(
            ...
            entities: [
                .arc(
                    center: .init(x: 0, y: 0), radius: 1, start: 10 * .pi / 180,
                    sweep: 40 * .pi / 180,
        ...
        func after(_ code: String) -> Double? {
            guard let i = lines.firstIndex(of: code) else { return nil }
            return Double(lines[i + 1])
        }
    ...
    static func sampleWithBlocksAndDims() -> Data {
        ...
        b.classTag("CDataSen")
        b.senFields(0, 0, 10, 0)  // top-level line
        b.classTag("CDataSunpou")
        b.base(layer: 3)  // dimension
        b.senFields(0, 0, 10, 0)
        b.mojiFields("100")  // dim line + value text
        b.u16(0)
        b.senFields(0, 0, 0, 1)
        b.senFields(10, 0, 10, 1)  // sxf mode + 2 witness lines
        for _ in 0..<4 { b.tenFields(0, 0) }  // 4 arrow/ref points
        b.classTag("CDataBlock")
        b.base(layer: 2)  // block insert → def 7
        b.f64(5)
        b.f64(6)
        b.f64(1)
        b.f64(1)
        ...
        b.u16(1)  // 1 member
        b.classTag("CDataSen")
        b.senFields(0, 0, 5, 5)  // member line
        return b.d
    }
    ...
    func blocksAndDims() throws {
        ...
        // only the top-level line (dim parts + block members excluded)
        ...
        guard
            let ins = dwg.entities.first(where: {
                if case .insert = $0 { return true } else { return false }
            }),
            case .insert(let num, let at, _, _, _, _, _) = ins
        else {
            Issue.record("no insert")
            return
        }
        #expect(num == 7 && at.x == 5 && at.y == 6)
        // the dimension decomposes into parts (line + text + witnesses + arrows)
        guard
            let dim = dwg.entities.first(where: {
                if case .dimension = $0 { return true } else { return false }
            }),
            case .dimension(let parts, _) = dim
        else {
            Issue.record("no dimension")
            return
        }
